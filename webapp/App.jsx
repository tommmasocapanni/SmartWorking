...
export default function WorkRadar() {
  ...
  useEffect(function(){
    ...
    // Controlla e ripristina la subscription push se si era persa
    if(ps==="granted"){
      (async function(){
        try{
          ...
          var sub=await reg.pushManager.getSubscription();
          if(!sub){
            // Subscription persa (es. iOS ha scaduto il token) — ri-subscribe silenziosamente
          ...
          // Ri-registra sempre la subscription sul server (il server potrebbe essersi riavviato)
          var h={"Content-Type":"application/json"};
