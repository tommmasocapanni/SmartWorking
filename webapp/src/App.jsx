...
function groupIntoThreads(jobs) {
  ...
  return order.map(key => {
    ...
    const top = emails[0];
    return {
      id: top.id, title: top.title, desc: top.desc,
      from: top.from, box: top.box, date: top.date,
      status: top.status, note: top.note||"",
      budget: top.budget||"", tags: top.tags||[],
      deadline: top.deadline||"", pinned: top.pinned||false,
...
export default function App() {
  ...
  const doSync = useCallback(async (silent=false) => {
    ...
    try {
      ...
      // Loop a chunk: continua finché il server dice hasMore=true
      ...
      while(true) {
        chunk++;
        if(!silent) setLoadMsg(`Caricamento... (${totalFetched} email)`);
        const res = await fetch(`${cfg.serverUrl}/sync`, {
          method:"POST", headers:h,
          body: JSON.stringify({
            email:cfg.email, password:cfg.password,
            host:cfg.host, port:cfg.port,
            boxes:selBoxes, lastUIDs:currentUids,
            limit: 30
          })
        });
        ...
        if(data.lastUIDs) {
          currentUids = {...currentUids, ...data.lastUIDs};
          setUids(currentUids);
          ls.set(STORE.uids, currentUids);
        }

        // Se non ci sono altri chunk da caricare, esci
        if(!data.hasMore) break;
      }

      const now = new Date().toISOString();
      setLastSync(now); ls.set(STORE.lastsync, now);
      if(!silent) setLoadMsg(totalFetched > 0 ? `+${totalFetched} nuove` : "Nessuna novità");
    } catch(e) {
