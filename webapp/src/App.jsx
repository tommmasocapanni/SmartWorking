...
function tagColor(tag) {
  let i = 0;
  for (const c of tag) i += c.charCodeAt(0);
...
.logo { font-family: 'DM Mono', monospace; font-size: 13px; letter-spacing: .08em; display: flex; align-items: center; gap: 10px; }
...
/* STATS */
.stats { display: flex; gap: 1px; background: var(--border2); border: 1px solid var(--border2); border-radius: var(--r); overflow: hidden; margin-bottom: 28px; }
...
.search {
  font-family: 'DM Mono', monospace; font-size: 12px; background: var(--surface);
...
.error-bar {
  ...
  font-family: 'DM Mono', monospace; font-size: 11px; color: var(--red); margin-bottom: 14px;
}
...
.swipe-bg { position: absolute; inset: 0; border-radius: var(--r); display: flex; align-items: center; padding: 0 18px; pointer-events: none; }
...
.thread-s1 { position: absolute; bottom: -5px; left: 8px; right: 8px; height: 100%; background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); z-index: -1; }
.thread-s2 { position: absolute; bottom: -10px; left: 16px; right: 16px; height: 100%; background: var(--surface2); border: 1px solid var(--border); border-radius: var(--r); z-index: -2; }
.thread-badge {
  display: inline-flex; align-items: center; gap: 3px; font-family: 'DM Mono', monospace;
...
.card-from { font-size: 10px; color: var(--text3); font-family: 'DM Mono', monospace; margin-bottom: 7px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.card-note { font-size: 10px; color: var(--text3); font-style: italic; margin-bottom: 7px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.dl-badge { font-family: 'DM Mono', monospace; font-size: 10px; padding: 2px 7px; border-radius: 20px; border: 1px solid; }
...
.modal-meta { display: flex; gap: 20px; flex-wrap: wrap; padding-bottom: 14px; border-bottom: 1px solid var(--border); margin-bottom: 14px; }
...
.section-label { font-size: 9px; text-transform: uppercase; letter-spacing: .1em; color: var(--text3); margin-bottom: 7px; margin-top: 14px; }
.modal-text { font-size: 12px; color: var(--text2); line-height: 1.75; white-space: pre-wrap; }
.modal-input {
  font-family: 'DM Mono', monospace; font-size: 12px; width: 100%; background: var(--bg);
...
.modal-textarea {
  width: 100%; font-family: 'DM Sans', sans-serif; font-size: 12px; background: var(--bg);
...
.t-email-body { padding: 10px 13px; font-size: 11px; color: var(--text2); line-height: 1.7; white-space: pre-wrap; border-top: 1px solid var(--border); }
...
.tab-btn { flex: 1; padding: 7px; font-size: 11px; font-weight: 500; background: var(--surface2); color: var(--text3); border: none; font-family: 'DM Sans', sans-serif; cursor: pointer; }
...
.field input, .field select { font-family: 'DM Mono', monospace; font-size: 12px; width: 100%; background: var(--bg); border: 1px solid var(--border); border-radius: var(--rs); padding: 7px 10px; color: var(--text); outline: none; transition: border-color .15s; }
.field input:focus, .field select:focus { border-color: var(--text); }
.field input::placeholder { color: var(--text3); }
.setup-note { font-size: 11px; color: var(--text3); line-height: 1.6; background: var(--bg); border-radius: var(--rs); padding: 9px; border: 1px solid var(--border); }
...
.box-row { display: flex; align-items: center; gap: 10px; padding: 9px 11px; background: var(--bg); border: 1px solid var(--border); border-radius: var(--rs); cursor: pointer; }
.box-row:hover { border-color: var(--border2); }
.box-row.on { background: var(--surface); border-color: var(--text); }
.box-check { width: 15px; height: 15px; border-radius: 4px; border: 1.5px solid var(--border2); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.toast-undo { font-family: 'DM Mono', monospace; font-size: 11px; background: rgba(128,128,128,.2); border: 1px solid rgba(128,128,128,.3); border-radius: 6px; padding: 3px 9px; cursor: pointer; color: inherit; }
.toast-undo:hover { background: rgba(128,128,128,.35); }
.toast-bar { position: absolute; bottom: 0; left: 0; height: 2px; background: rgba(128,128,128,.4); border-radius: 0 0 var(--r) var(--r); animation: toastBar 5s linear forwards; }
  ...
  return (
    <div className="card-wrap" style={{ marginBottom: isThread ? 10 : 0 }}>
      ...
      >
        {isThread && (
          <>
            <div className="thread-s1"/>
            {thread.count > 2 && <div className="thread-s2"/>}
          </>
        )}
...
export default function App() {
  ...
  async function loadBoxes() {
    ...
    try {
      const h = { "Content-Type": "application/json" };
      if (cfg.secret) h["Authorization"] = `Bearer ${cfg.secret}`;
      const r = await fetch(`${cfg.serverUrl}/boxes`, {
        method:  "POST",
        headers: h,
    ...
    <>
      ...
      <div className="app">
        ...
        >
          ...
          <div className="stats">
            {[
              { l: "Thread",    n: stats.totale    },
              { l: "Nuovi",     n: stats.nuovi     },
              { l: "Applicato", n: stats.applicato },
              { l: "Email",     n: stats.email     },
            ].map(s => (
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <div className="modal-actions-right">
                <button
                  className={`btn btn-ghost${selected.pinned ? " active" : ""}`}
          ...
          <div className="setup-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="tabs">
              {["server", "email", "cartelle", "sync"].map(t => (
                <button
                  key={t}
                  className={`tab-btn${setupTab === t ? " active" : ""}`}
                  onClick={() => { setSetupTab(t); if (t === "cartelle" && avBoxes.length === 0) loadBoxes(); }}
                >
                  {t}
                </button>
              ))}
            ...
            {setupTab === "cartelle" && <>
              {boxLoading && (
                <div style={{ fontFamily: "DM Mono,monospace", fontSize: 11, color: "var(--text3)", padding: 14, textAlign: "center" }}>
                  Caricamento...
                </div>
              )}
